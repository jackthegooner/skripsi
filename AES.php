...
class AES {
    ...
    static $caller;
    /** @var int Number of rounds in AES algorithm (Nr) {10, 12, 14}*/
    ...
    public static function sbox() {
        return (new AES())->s_box;
    }
    /**
     * Creates inverted S-Box Table
     */
    public function createInvertedSBox() {
    ...
    public function getMatrixFromState($array = false)
    {
        if($array == false) {
            $array = $this->state;
        }
        ...
        for($i = 0; $i < count($array); $i++) {
            $data = str_split(sprintf("%08s", dechex($array[$i])), 2);
            for($j = 0; $j < 4; $j++) {
                $strState[$j][$i] = $data[$j];
            }
        }
    ...
    public function encrypt($content, $key) {
        ...
        for ($round = 1; $round < $this->number_of_rounds; $round++) {
            ...
            $oldStateMatrix = $this->getMatrixFromState();
            $this->subBytes();

            if($round == 1) {
                self::$caller->setCalculatedStepData('subbytes', [
                    'state' => $oldStateMatrix,
                    'newState' => $this->getMatrixFromState()
                ]);
            }
            ...
            $this->shiftRows();
            if($round == 1) {
                self::$caller->setCalculatedStepData('shiftrows', [
                    'state' => $oldStateMatrix,
                    'newState' => $this->getMatrixFromState()
                ]);
            }
            ...
            $this->mixColumns();
            if($round == 1) {
                self::$caller->setCalculatedStepData('mixcolumns', [
                    'state' => $oldStateMatrix,
                    'newState' => $this->getMatrixFromState()
                ]);
            }
            ...
            $oldStateMatrix = $this->getMatrixFromState();
            $roundKey = $this->createRoundKey($round, $expand_key);
            $this->addRoundKey($roundKey);
            if($round == 1) {
                self::$caller->setCalculatedStepData('addroundkey', [
                    'state' => $oldStateMatrix,
                    'newState' => $this->getMatrixFromState(),
                    'key' => $this->getMatrixFromState($roundKey)
                ]);
            }
        }
        $this->subBytes();
    ...
    private function keyExpansion($key) {
        ...
        for ($i=$this->key_size; $i<$this->block_size * ($this->number_of_rounds + 1); $i++) {
            ...
            if($i == $this->key_size) {
                $data['firstColumn'] = $this->bytesArrayFromWord($expanded_key[$i - $this->key_size]);
                $data['lastColumn'] = $this->bytesArrayFromWord($this->subWord($this->rotWord($expanded_key[$i - 1])));
                $data['rconColumn'] = $this->bytesArrayFromWord($this->round_constants[$i / $this->key_size -1]);

                $data['resultColumn'] = $this->bytesArrayFromWord($expanded_key[$i - $this->key_size] ^ $temp);
            }

            $expanded_key[$i] = $expanded_key[$i - $this->key_size] ^ $temp;
        }
        /* koment */
        // self::$caller->setCalculatedStepData('keyexpansion', $data); 

        return $expanded_key;
    }

    public function bytesArrayFromWord($word)
    {
        return str_split(sprintf("%08s", dechex($word)), 2);
    }
    ...
    private function createRoundKey($round, $key) {
        return array(
            ...
            $key[self::WORD_LENGTH * $round + 1],
            $key[self::WORD_LENGTH * $round + 2],
            $key[self::WORD_LENGTH * $round + 3]
        );
    }
    /**
...
//        dd(dechex($word),dechex(($this->s_box[(( $word >> 24 ) & 0x000000FF)] << 24)));
        if (!$invers)
